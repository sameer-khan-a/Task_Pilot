...
const res = await axios.get(`${process.env.VITE_BACKEND_URL}/api/invitations/my`, {
...
const fetchTaskNotifications = async () => {
    try {
        ...
        const res = await axios.get(`${process.env.VITE_BACKEND_URL}/api/notifications`, {
            headers: {Authorization: `Bearer ${token}`},
        });
...
const markNotificationAsRead = async (notificationId) => {
    try {
        const token = localStorage.getItem('token');
        await axios.put(`${process.env.VITE_BACKEND_URL}/api/notifications/${notificationId}/read`, null,  {
            headers: {Authorization: `Bearer ${token}`},
        });
          if(soundRef.current){
...
const deleteNotification = async (notificationId) => {
    try{
        const token = localStorage.getItem('token');
        await axios.delete(`${process.env.VITE_BACKEND_URL}/api/notifications/${notificationId}`, {
            headers: {Authorization: `Bearer ${token}`},
        });
...
await axios.post(
`${process.env.VITE_BACKEND_URL}/api/invitations/${invitationId}/respond`,
{response: action},
{
headers: {Authorization: `Bearer ${token}`},
}
